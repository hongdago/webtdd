...
class ListViewTest(TestCase):
    ...
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post('/lists/%d/' % (correct_list.id,),data={'text':'textey'})
        self.assertEqual(Item.objects.count(),1)
        new_item = Item.objects.first()
        self.assertEqual(new_item.text,'textey')
    ...
    def post_invalida_input(self):
        list_=List.objects.create()
        return self.client.post('/lists/%d/' % (list_.id,),data={'text':''})

    def test_for_invalid_input_nothing_saved_to_db(self):
        self.post_invalida_input()
        self.assertEqual(Item.objects.count(),0)

    def test_for_invalid_input_renders_list_template(self):
        response=self.post_invalida_input()
    ...
    def test_duplicate_item_validation_errros_end_up_on_lists_page(self):
...
class NewListTest(TestCase):
    ...
    def test_ubvalid_list_items_arent_saved(self):
    ...
    def tet_for_invalid_input_renders_home_template(self):
        response=self.client.post('/lists/new',data={'text':''})
        self.assertEqual(response.status_code,200)
        self.assertTemplateUsed(response,'home.html')

    def tet_validation_errors_are_shown_on_home_page(self):
        response=self.client.post('/lists/new',data={'text':''})
        self.assertContains(response,escape(EMPTY_LIST_ERROR))
